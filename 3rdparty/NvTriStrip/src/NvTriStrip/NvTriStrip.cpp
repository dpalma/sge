...
{
	//delete strips
	for(NvStripInfoVec::size_type i = 0; i < tempStrips.size(); i++)
	{
		for(NvFaceInfoVec::size_type j = 0; j < tempStrips[i]->m_faces.size(); j++)
		{
	...
	else
	{
		...
		*numGroups = numSeparateStrips; //for the strips
		...
		for(unsigned int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
            ...
            for(IntVec::size_type i = startingLoc; i < stripIndices.size(); i++)
				{
					if(stripIndices[i] == -1)
						break;
				}
		...
		if(tempFaces.size() != 0)
		{
			...
			int indexCtr = 0;
			for(NvFaceInfoVec::size_type i = 0; i < tempFaces.size(); i++)
			{
